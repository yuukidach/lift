...
use objc2_foundation::{
    MainThreadMarker, NSAttributedStringKey, NSDictionary, NSMutableDictionary, NSObject, NSPoint,
...
use crate::common::config::{
    ActiveWorkspaceLabel, MenuBarDisplayMode, MenuBarSettings, MenuBarWorkspaceScope,
    WorkspaceDisplayStyle, WorkspaceSelector, workspace_number_to_global_slot,
};
...
impl MenuIcon {
    ...
    pub fn update(
        ...
        let build = |inputs: &[WorkspaceRenderInput]| {
    ...
    fn update_display_views(
        ...
        let live_numbers = windows
            .iter()
            .map(|window| window.windowNumber())
            .filter(|number| status_window_displays.contains_key(&(*number as u32)))
            .collect::<Vec<_>>();
        self.display_views.retain(|number, view| {
            let keep = live_numbers.contains(number);
            if !keep {
        ...
        for window in windows.iter() {
            ...
            let Some(&display_index) = status_window_displays.get(&(window_number as u32)) else {
                continue;
            };
            let Some(layout) = layouts.get(display_index) else {
                continue;
            };
            let Some(content_view) = window.contentView() else {
                continue;
            };
            let view = self.display_views.entry(window_number).or_insert_with(|| {
                let view = MenuIconView::new(self.mtm);
            ...
            let outer_inset = ((bounds.size.width - reserved_width) / 2.0).max(0.0);
            let x = outer_inset + (reserved_width - layout.total_width).max(0.0);
            let y = (bounds.size.height - layout.total_height) / 2.0;
            view.setFrame(CGRect::new(
                CGPoint::new(x, y),
                CGSize::new(layout.total_width, layout.total_height),
            ));
            view.set_layout(layout.clone());
            view.setHidden(false);
            view.displayIfNeeded();
        }
    }

    fn clear_display_views(&mut self) {
...
fn workspace_render_inputs(
    ...
    let grouped_workspaces = workspaces.iter().cloned().enumerate().map(|(index, workspace)| {
    ...
    grouped_workspaces
        ...
        .map(|(display_group, workspace)| match settings.display_style {
            WorkspaceDisplayStyle::Layout => WorkspaceRenderInput {
                workspace,
                label: String::new(),
                show_windows: true,
                app_icon: None,
            ...
            WorkspaceDisplayStyle::Label => {
                let app_icon = primary_app_icon(&workspace);
...
fn display_index_for_window(window: CGRect, displays: &[MenuBarDisplayData]) -> Option<usize> {
    ...
    displays
        ...
        .or_else(|| {
            displays
                ...
                .map(|(index, display)| {
                    let frame = display.frame;
                    let left = window.origin.x.max(frame.origin.x);
                    let top = window.origin.y.max(frame.origin.y);
                    let right = (window.origin.x + window.size.width)
                        .min(frame.origin.x + frame.size.width);
                    let bottom = (window.origin.y + window.size.height)
                        .min(frame.origin.y + frame.size.height);
                    (index, (right - left).max(0.0) * (bottom - top).max(0.0))
                })
...
fn draw_menu_layout(cg: &CGContext, bounds: CGRect, layout: &MenuIconLayout, x_offset: f64) {
    CGContext::save_g_state(Some(cg));
    CGContext::clear_rect(Some(cg), bounds);
    CGContext::translate_ctm(Some(cg), x_offset, 0.0);
    ...
    for workspace in &layout.workspaces {
        ...
        if workspace.show_windows {
            ...
            CGContext::set_stroke_color_with_color(Some(cg), Some(layout_border.as_ref()));
            ...
            CGContext::set_fill_color_with_color(Some(cg), Some(label_background.as_ref()));
            CGContext::set_alpha(Some(cg), workspace.fill_alpha);
            CGContext::fill_path(Some(cg));
            CGContext::set_alpha(Some(cg), LABEL_ACTIVE_INDICATOR_ALPHA);
            CGContext::set_fill_color_with_color(Some(cg), Some(active_background.as_ref()));
    ...
    impl MenuIconView {
        ...
        fn draw_rect(&self, _dirty_rect: NSRect) {
            ...
            if let Some(context) = NSGraphicsContext::currentContext() {
                ...
                draw_menu_layout(cg_context.as_ref(), bounds, &layout, 0.0);
            }
        }
    }
);
...
mod tests {
    ...
    fn status_windows_map_to_displays_with_negative_origins() {
        ...
        let left = CGRect::new(CGPoint::new(-642.0, -205.0), CGSize::new(148.0, 24.0));
        let main = CGRect::new(CGPoint::new(1918.0, 0.0), CGSize::new(148.0, 24.0));

        assert_eq!(display_index_for_window(left, &displays), Some(0));
        assert_eq!(display_index_for_window(main, &displays), Some(1));
    }
}
